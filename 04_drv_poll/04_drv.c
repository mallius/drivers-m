...
#include <linux/poll.h>		//poll-->poll_wait
...
int fourth_open (struct inode *inode, struct file *file)
{
	printk("fourth_open: \n");
...
ssize_t fourth_read (struct file *file, char __user *user, size_t size, loff_t *loff)
{
	printk("fourth_read:\n");
...
ssize_t fourth_write (struct file *file, const char __user *user, size_t size, loff_t *loff)
{
	printk("This is fourth_write.\n");

	return 0;
}

int fourth_release (struct inode *inode, struct file *file)
{
...
static unsigned int fourth_poll(struct file *file, poll_table *wait)
{
...
int major;
struct class 		*fourth_class;
struct class_device 	*fourth_class_device;

int fourth_init(void)
{
	major = register_chrdev(0, "fourth_drv",&f_ops);	//名字不重要 cat /proc/devices时显示252设备号和名字

	fourth_class = class_create(THIS_MODULE, "fourth_key_drv");

	fourth_class_device = class_device_create(fourth_class, NULL, MKDEV(major,0), NULL, "keys");
...
{
	unregister_chrdev(major, "fourth_drv");       //名字不重要，"sec"也可以

	class_device_unregister(fourth_class_device);	//先unregister, 再destroy
